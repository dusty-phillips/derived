...
"
  ...
  assert result == Error(derived.GenerationError("callback failed for some reason"))
}
...
"
  ...
  assert result == Error(derived.GenerationError("failed to generate for Baz"))
}
...
"
  ...
  assert result == Error(derived.ParseError(ast.UnexpectedEndOfFile))
}
...
"
  ...
  let result = derived.parse(source)
